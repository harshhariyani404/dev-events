...
export const createBooking = async ({ eventId, slug, email}: { eventId: string; slug: string; email:string}) => {
    try{
        ...
        // Verify event exists and get canonical slug
        const eventDoc = await Event.findById(objectId).lean();
        if (!eventDoc) {
            return { success: false, error: 'Event not found' };
        }

        const normalizedSlug = (slug && typeof slug === 'string') ? slug.toLowerCase().trim() : eventDoc.slug;
        ...
        // Create the booking (include slug for easier queries)
